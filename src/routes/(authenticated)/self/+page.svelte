...
{#if session}
  <div class="max-w-2xl mx-auto mt-10 space-y-8">
    <div class="bg-white rounded-2xl shadow p-6 space-y-2">
      ...
      <p>
        <strong>Alias:</strong>
        {session.user.user_metadata.nickname || "No definido"}
      </p>
    </div>
        ...
        <div class="space-y-2">
          <form method="POST" action="?/convert_provider">
            <button
              ...
              name="provider"
              value="github">Convertir con GitHub</button
            >
          </form>
...
{#if form?.message}
  <div
    class="max-w-2xl mx-auto mt-4 text-green-700 bg-green-100 p-3 rounded-md shadow"
  >
    {form.message}
  </div>
{/if}
...
{#if form?.error}
  <div
    class="max-w-2xl mx-auto mt-4 text-red-700 bg-red-100 p-3 rounded-md shadow"
  >
    {form.error}
  </div>
{/if}
...
{#if form?.verify}
  <div class="max-w-2xl mx-auto mt-6 bg-white rounded-2xl shadow p-6">
    <form method="POST" action="?/verify_otp" class="space-y-4">
      <input
        name="otp"
        placeholder={`OTP enviado a ${form?.phone ?? form?.email}`}
        class="input"
      />
